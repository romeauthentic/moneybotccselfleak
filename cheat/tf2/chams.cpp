...
namespace features
{
	...
	void c_material::init( const char* name, const char* buf ) {
		//fuck this nigger
		m_keyvalues = ( KeyValues* )malloc( 0x1000 ); //give it a full page of mem, suck a dick valve
		
		//cock
		m_keyvalues->init( name );
		...
		if( !m_mat ) {
			printf( "dude what the fuck\n" );
		}
		else {
			printf( "created (%08x)\n", m_mat );
		}

		m_mat->IncrementReferenceCount( ); //not in tf2
	}
