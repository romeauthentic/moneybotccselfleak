...
namespace features
{
	...
	void c_ragebot::c_lby_breaker::update_animstate( ) {
        ...
        g_ctx.m_local->get_animstate( )->m_flFeetYawRate = 0.f;
		...
		if( g_cheat.m_lagmgr.get_state( ) ) {
			...
			g_ctx.m_local->ce( )->SetupBones( g_ctx.m_fake_matrix, 128, BONE_USED_BY_ANYTHING, 0.f );
			g_ctx.m_local->get< byte >( 0x274 ) = backup;
			g_ctx.m_fake_absyaw = g_ctx.m_local->get_animstate( )->m_flGoalFeetYaw;
	
			memcpy( g_ctx.m_local->get_animdata( ).m_poseparams.data( ),
				g_ctx.m_local->m_flPoseParameter( ),
				sizeof( float ) * 24 );
		}
	...
	bool c_ragebot::c_antiaim::run_edge_dtc( ) {
		...
		final_yaw = edge_yaw;
		if( g_cheat.m_lagmgr.get_state( ) ) {
			final_yaw = edge_yaw + desync_delta( );
		}
	...
	bool c_ragebot::c_antiaim::run_freestanding( int player, bool set_angles ) {
		if( !g_ctx.m_local->get_animstate( )->m_bOnGround )
			return false;
		...
		if( is_fake ) {
			float sign = !m_direction ? 1.f : -1.f;
			aim_angle.y = g_ctx.m_local->get_animstate( )->m_flGoalFeetYaw - desync_delta( ) * sign;
		}
		else {
		...
		return g_settings.rage.edge_detection( );
	}
	...
	void c_ragebot::c_antiaim::run_fakewalk( ) {
		...
		if( !( g_ctx.m_local->m_fFlags( ) & FL_ONGROUND ) || g_ctx.m_local->m_fFlags( ) & FL_DUCKING )
			return;
		...
		if( !g_settings.rage.fakewalk_ticks ) {
			auto state = g_ctx.m_local->get_animstate( );
			...
			float speed = g_ctx.m_local->m_vecVelocity( ).length2d( );
			 ...
			 max_speed *= ( float )( g_settings.rage.fakewalk_ticks * 0.01f );
	...
	float c_ragebot::c_antiaim::get_yaw( bool no_jitter ) {
		...
		if( is_fake && g_ctx.m_local->m_fFlags( ) & FL_ONGROUND )
			return g_ctx.m_local->get_animstate( )->m_flGoalFeetYaw + desync_delta( ) * sign;
	...
	void c_ragebot::c_antiaim::run( ) {
		...
		if( !g_settings.rage.edge_dtc_priority ) {
			freestanding = run_freestanding( -1, g_settings.rage.edge_detection( ) );
			dir_updated = true;
		...
		if( !dir_updated )
			run_freestanding( -1, g_settings.rage.edge_detection( ) );
		...
		if( g_settings.rage.fake_yaw && g_ctx.m_local->m_fFlags( ) & FL_ONGROUND ) {
			if( !m_cmd->m_sidemove && !m_cmd->m_forwardmove ) {
				g_ctx.get_last_cmd( )->m_sidemove = ( m_cmd->m_cmd_nr % 2 ) ? 2.f : -2.f;
			}
		}
