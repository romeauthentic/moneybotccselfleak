...
void c_context::on_cmove_end( user_cmd_t* cmd ) {
	...
	if( send ) {
		...
		if( !g_cheat.m_lagmgr.has_fired( ) )
			m_last_realangle = cmd->m_viewangles;
		else
			m_last_fakeangle = m_last_realangle;
		//m_last_realangle.x = m_last_fakeangle.x;
	}

	if( !g_cheat.m_lagmgr.get_state( ) && g_cheat.m_ragebot.m_antiaim->get_next_update( ) > 1 && !g_cheat.m_lagmgr.has_fired( ) ) {
	...
	if( g_cheat.m_lagmgr.get_sent( ) > 1 ) {
		m_thirdperson_angle = m_last_fakeangle;
	}
	...
	if( ( cmd->m_buttons & IN_ATTACK ) && m_local->can_attack( ) ) {
		...
		if( !weap->is_knife( ) && weap->m_iItemDefinitionIndex( ) != WEAPON_ZEUS && !weap->is_grenade( ) ) {
