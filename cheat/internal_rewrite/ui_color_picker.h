	...
	public:
		...
		virtual void render( ) override {
			...
			if ( m_active ) {
				...
				ui_draw_rect( x, y, BOX_WIDTH + 21, BOX_HEIGHT + 1, ui_get_bg_col( ) );
				...
				auto draw_slider_hue = [&]( ) {
					for ( int i{ }; i < 100; ++i ) {
						auto cur_col = clr_t::from_hsb( float( i ) * 0.01f, 1.0f, 1.0f);
				...
				auto is_alpha_slider_hovered = [&]( ) -> bool {
					return mouse_x > x + 0 && mouse_x < x + 122
						&& mouse_y > y + 110 && mouse_y < y + 124;
				};

				auto draw_slider_alpha = [&]( ) {
					clr_t col_bg1 = { 204, 204, 204, 255 };
					clr_t col_bg2 = { 128, 128, 128, 255 };

					ui_draw_rect( x + 2, y + 110, 119, 12, col_bg1 );

					int yi = 0;
					float max_x = x + 121;
					float max_y = y + 12;
					for( float y1 = y; y1 < max_y; y1 += 4.f, yi++ ) {
						for( float x1 = x + 2 + ( yi & 1 ) * 4.f; x1 < max_x; x1 += 4.f * 2.0f ) {
							ui_draw_rect( x1, y1 + 110, 4, 4, col_bg2 );
						}
					}
					...
					int a_pos = ( int )( float( m_alpha ) * 119.f / 255.f );
				...
				if ( is_alpha_slider_hovered( ) && g_input.is_key_pressed( KEYS_MOUSE1 ) ) {
					int delta = std::clamp( mouse_x - x, -1, 120 );
					m_alpha = ( int )( float( delta ) * 255.f / 119.f );
				}
                ...
                // draw alpha
				if ( m_width != 120 ) { // yep.. dont want to draw this on the cham color pickers cuz it looks weird
					clr_t col_bg1 = { 204, 204, 204, 255 };
					clr_t col_bg2 = { 128, 128, 128, 255 };

					ui_draw_rect( x, y, m_width, m_height, col_bg1 );

					int yi = 0;
					float max_x = x + m_width;
					float max_y = y + m_height;
					for ( float y1 = y; y1 < max_y; y1 += 4.f, yi++ ) {
						for ( float x1 = x + ( yi & 1 ) * 4.f; x1 < max_x; x1 += 4.f * 2.0f ) {
							ui_draw_rect( x1, y1, 4, 4, col_bg2 );
						}
					}
				}
