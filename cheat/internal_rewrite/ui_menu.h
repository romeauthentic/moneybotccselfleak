...
namespace ui
{
	...
	public:
		...
		virtual void render( ) override {
			...
			clr_t top_clr = ui_get_bg_col( );

			ui_draw_rect( m_x + 1, m_y + 1, m_width - 1, top_height - 2, top_clr );
			if( is_hovered( ) ) {
				clr_t hovered_col = top_clr;
				if( theme == 1 ) {
					float brightness = top_clr.brightness( );
					float hue = top_clr.hue( );
					float sat = top_clr.saturation( );

					brightness = std::clamp( brightness * ( theme == 0 ? 1.1f : 0.8f ), 0.f, 1.f );
					hovered_col = clr_t::from_hsb( hue, sat, brightness );
				}
				else hovered_col = hovered_col * 1.2f;

				ui_draw_rect( m_x + 1, m_y + 1, m_width - 1, top_height - 2, hovered_col );
			}
