...
namespace ui
{
	...
	public:
		...
		virtual void render( ) override {
			...
			for( int i{ }; i < 8; ++i ) {
				clr_t col = ui_get_bg_col( ) * ( theme == 0 ? ( 0.72f + i * 0.04f ) : ( 0.93 + i * 0.01f ) );
			...
			if( m_max_height && get_total_item_height( ) > m_height ) {
				const int height = get_total_height( ) - 20;
				...
				int slider_pos = static_cast< int >( slider_step * m_scroll_offset );
