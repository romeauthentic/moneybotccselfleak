	...
	public:
		...
		void draw_items( const int& x, const int& y ) {
			...
			for ( size_t i = items.size( ) > m_max_items ? m_curr_scroll : 0;
				...
				draw_box( x, y + offset, it->m_name.c_str( ) );

				if ( is_hovered( offset ) ) {
					hovered = offset;
					if ( g_input.is_key_just_pressed( KEYS_MOUSE1 ) ) {
						m_selected_item = it;
		...
		void input( ) {
			...
			if ( is_hovered( ) && g_input.is_key_just_pressed( KEYS_MOUSE1 ) ) {
				if ( !m_mouse_held ) {
					m_active = !m_active;
				}
				m_mouse_held = true;
			...
			if ( !is_hovered( ) && g_input.is_key_just_pressed( KEYS_MOUSE1 ) && !is_any_item_hovered( ) ) {
				m_active = false;
			}
		...
		virtual void render( ) override {
			...
			if ( m_selected_item ) {
				ui_draw_string( x + m_width / 2, y + 14, true, ui_get_text_col( ), m_selected_item->m_name.c_str( ) );
			}
