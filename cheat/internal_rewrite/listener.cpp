...
{	
	...
	void bullet_impact( IGameEvent* e ) {
		if( !g_csgo.m_panic && e ) {
			...
			if( player_id == g_csgo.m_engine( )->GetLocalPlayer( ) ) {
				...
				// search for FX_GunshipImpact 
				// 55 8B EC 83 EC 18 8B 45 08 8D 55 F4 F3 0F 10 15 ? ? ? ? 
				/*static auto FX_GunshipImpactEx = pattern::first_code_match< void* >( g_csgo.m_chl.dll( ),
					xors( "55 8B EC 83 EC 18 8B 45 08 8D 55 F4 F3 0F 10 15" ) );

				CEffectData effect_data;
				effect_data.m_vOrigin = impact;

				reinterpret_cast< void( __cdecl* )( const CEffectData& data ) >( FX_GunshipImpactEx )( effect_data );*/
	...
	void round_start( IGameEvent* e ) {
		...
		for ( int i{ 64 }; i < g_csgo.m_entlist( )->GetHighestEntityIndex( ); ++i ) {
			...
			if ( client_class && client_class->m_class_id  == CPlantedC4 && ( strstr( client_class->m_name, xors( "Planted" ) ) ) ) {
				static const auto m_bBombDefused = g_netvars.get_netvar( fnv( "DT_PlantedC4" ), fnv( "m_bBombDefused" ) );
				*(bool*) ( uintptr_t( ent ) + m_bBombDefused ) = true;
		...
		if( g_ctx.run_frame( ) ) {
			...
			if( g_ctx.m_local->m_iAccount( ) > 1000 ) {
				...
				if( g_settings.misc.autobuy.zeus( ) )
					g_csgo.m_engine( )->ClientCmd_Unrestricted( xors( "buy taser" ) );
	...
	void player_death( IGameEvent *evt ) {
		if( !g_csgo.m_panic && evt && g_settings.visuals.kill_effect( ) ) {
			int user_id = evt->GetInt( xors( "userid" ) );
			int attacker = evt->GetInt( xors( "attacker" ) );

			int attacker_id = g_csgo.m_engine( )->GetPlayerForUserID( attacker );
			if( attacker_id == g_csgo.m_engine( )->GetLocalPlayer( ) && user_id != attacker_id ) {
				g_ctx.m_local->m_flHealthShotBoostExpirationTime() = g_csgo.m_globals->m_curtime + 1.f;
			}
		}
	...
	void item_purchase( IGameEvent *evt ) {
		if( !g_settings.misc.log_purchases( ) )
			return;

		auto ent = (c_base_player *)g_csgo.m_entlist( )->GetClientEntity( g_csgo.m_engine( )->GetPlayerForUserID( evt->GetInt( xors( "userid" ) ) ) );
		if( !ent )
			return;

		if( ent == g_ctx.m_local || ent->m_iTeamNum() == g_ctx.m_local->m_iTeamNum() )
			return;
		...
		char print_str[ 256 ];
		sprintf_s( print_str, "[\3moneybot\1] %s purchased %s", name, evt->GetString( "weapon" ) );
