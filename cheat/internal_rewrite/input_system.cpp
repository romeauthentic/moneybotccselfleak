	...
	switch ( key_event ) {
		case KEYDOWN: {
			if ( is_valid_key( key ) ) {
				...
				m_timestamps[ key ] = g_csgo.m_globals->m_framecount;
			}
			return true;
		}
		case KEYUP: {
		...
		case LBUTTONDOWN:
			...
			m_timestamps[ KEYS_MOUSE1 ] = g_csgo.m_globals->m_framecount;
			return true;
		case LBUTTONUP:
		...
		case RBUTTONDOWN:
			...
			m_timestamps[ KEYS_MOUSE2 ] = g_csgo.m_globals->m_framecount;
			return true;
		case RBUTTONUP:
		...
		case MBUTTONDOWN:
			...
			m_timestamps[ KEYS_MOUSE3 ] = g_csgo.m_globals->m_framecount;
			return true;
		case MBUTTONUP:
		...
		case XBUTTONDOWN: {
			...
			m_timestamps[ pressed_xbutton ? KEYS_MOUSE5 : KEYS_MOUSE4 ] = g_csgo.m_globals->m_framecount;
			return true;
		}
		case XBUTTONUP: {
			bool pressed_xbutton = static_cast<bool>( HIWORD( key ) - 1 ); //should result in mouse4 as false, and mouse5 as true
...
bool c_input_manager::is_key_pressed( int key ) {
	return ( is_valid_key( key ) && m_pressed_keys[ key ] );
}

bool c_input_manager::is_key_just_pressed( int key ) {
	return ( is_valid_key( key ) && m_pressed_keys[ key ] && ( m_timestamps[ key ] + 1 == g_csgo.m_globals->m_framecount ) );
}
...
char c_input_manager::get_pressed_char( int* out ) {
	...
	for ( size_t i{ }; i < KEYS_MAX; ++i ) {
		if ( is_key_pressed( VirtualKeys_t( i ) ) ) {
			if ( ( i >= KEYS_A && i <= KEYS_Z )
				|| ( i >= KEYS_N0 && i <= KEYS_N9 )
				|| ( i >= KEYS_LEFT && i <= KEYS_DOWN )
				|| ( i >= KEYS_PRIOR && i <= KEYS_NEXT ) ) {
				pressed_character = i;
			}
		}
	}
	...
	if ( pressed_character ) {
		...
		if ( is_key_pressed( KEYS_SHIFT ) ) {
			...
			if ( pressed_character >= KEYS_LEFT
				&& pressed_character <= KEYS_DOWN )
				return char( pressed_character );

			if ( pressed_character >= KEYS_PRIOR
				&& pressed_character <= KEYS_NEXT )
				return char( pressed_character );
		}
	}
