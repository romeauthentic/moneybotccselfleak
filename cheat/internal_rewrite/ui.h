...
namespace ui
{
	...
	static void render( ) {
		if ( !was_setup ) {
			...
			menu = std::make_shared< ui::c_menu >( 10, 10, 580, 470, xors( "dollar hack" ),
				//don't ask
				date_str.c_str( ) );
#else
			menu = std::make_shared< ui::c_menu >( 10, 10, 580, 470, xors( "dollar hack" ),
			...
			auto tab_legit = std::make_shared< ui::c_tab_sheet >( xors( "legit" ), &icons::sprite_legit ); {
				...
				auto subtab_main = std::make_shared< ui::c_tab_sheet >( xors( "main" ) ); {
					...
					auto main_form = std::make_shared< ui::c_form >( 0, 0, 215, 0, xors( "legitbot" ) ); {
			...
			auto tab_rage = std::make_shared< ui::c_tab_sheet >( xors( "rage" ), &icons::sprite_rage ); {
				...
				auto subtab_aimbot = std::make_shared< ui::c_tab_sheet >( xors( "aimbot" ) ); {
					...
					auto accuracy_form = std::make_shared< ui::c_form >( 0, 0, 215, 157, xors( "accuracy" ), 157 ); {
						...
						//accuracy_form->add_item( std::make_shared< ui::c_checkbox >( 0, 0, xors( "resolve moving" ), &g_settings.rage.dbg_moving_resolver )
						//)->set_cond( [ ]( ) { return g_settings.rage.resolver; } );
						...
						auto visualize_checkbox = accuracy_form->add_item( std::make_shared< ui::c_checkbox >( 0, 0, xors( "visualize backtrack" ), &g_settings.rage.bt_visualize ) );
						visualize_checkbox->set_cond( [ ]( ) { return !g_settings.misc.hide_from_obs( ); } );
						visualize_checkbox->add_item( std::make_shared< ui::c_color_picker >( 175, 2, 16, &g_settings.rage.bt_col ) );

						accuracy_form->add_item( std::make_shared< ui::c_checkbox >( 0, 0, xors( "no spread" ), &g_settings.rage.compensate_spread ) );
					}
			...
			auto tab_visuals = std::make_shared< ui::c_tab_sheet >( xors( "visuals" ), &icons::sprite_visuals ); {
				...
				auto subtab_other = std::make_shared< ui::c_tab_sheet >( xors( "other" ) ); {
					...
					auto local_form = std::make_shared< ui::c_form >( 0, 0, 215, 0, xors( "local player" ), 215 ); {
						...
						local_form->add_item( std::make_shared< ui::c_checkbox >( 0, 0, xors( "sniper crosshair" ), &g_settings.visuals.snipers_crosshair( ) ) );
			...
			auto tab_misc = std::make_shared< ui::c_tab_sheet >( xors( "miscellaneous" ), &icons::sprite_misc ); {
				...
				auto subtab_general = std::make_shared< ui::c_tab_sheet >( xors( "general" ) ); {
					...
					auto helpers_form = std::make_shared< ui::c_form >( 0, 0, 215, 0, xors( "helpers" ), 235 ); {
						...
						// readd later with panorama execution shit
