...
namespace ui
{
	...
	public:
		...
		void input( ) {
			...
			static float last_key_input_time;

            if ( hovered && m_max > 1 ) {
				int key_pressed;
				g_input.get_pressed_char( &key_pressed );

				float delta_time = current_time - last_key_input_time;

				if ( fabs( delta_time ) > 0.2f ) {
					switch ( key_pressed ) {
					case KEYS_LEFT: *m_setting -= ( t )1; last_key_input_time = current_time; break;
					case KEYS_RIGHT: *m_setting += ( t )1; last_key_input_time = current_time; break;
					case KEYS_PRIOR: *m_setting -= ( t )10; last_key_input_time = current_time; break;
					case KEYS_NEXT: *m_setting += ( t )10; last_key_input_time = current_time; break;
					default: break;
					}
				}
            }
