	...
	public:
		...
		virtual int get_total_height( ) const override {
			...
			if( strlen( m_search_text ) ) {
				for( size_t i{ }; i < m_items->size( ); ++i ) {
					if( !strstr( m_items->at( i ).m_name.c_str( ), m_search_text ) )
						continue;
		...
		void draw_items( int x, int y ) {
			...
			for( size_t i{ }; i < m_items->size( ); ++i ) {
				...
				if( !strstr( item.m_name.c_str( ), m_search_text ) )
					continue;
				...
				ui_draw_string( x + 2, y + 3, false, ui_get_text_col( ), item.m_name.c_str( ) );

				if( g_input.is_key_just_pressed( KEYS_MOUSE1 ) && is_item_hovered( x, y ) )
